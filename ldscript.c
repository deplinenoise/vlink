/* $VER: vlink ldscript.c V0.13 (02.11.10)
 *
   ...
   Its name is build from the first section pattern found. */
{
  ...
  *np = '\0';
  ...
  addtail(&script_obj->sections,&dummy->n);
  return dummy;
}
#endif
      ...
      case 'A':
        if (c == 'A') {
          if (getchr() == 'T') {
            ...
            back(1);
            error(65,scriptname,getlineno(),getword());
          }
        }
        if (buf = getword()) {
...
static lword readmemparam(char *key)
{
  char *str;
  lword val = 0;
...
/* <memblockname> [(attr)]: o[rg] = <base address>, l[en] = <maxlen> */
{
  ...
  if (startofblock('{')) {
    while (str = getword()) {
      ...
      if (c == ':') {
        org = readmemparam("ORIGIN");
        if (getchr() == ',') {
          len = readmemparam("LENGTH");
            ...
            while (keyword = getpattern()) {
              if (check_command(gv,keyword,SCMDF_GLOBAL|SCMDF_IGNORE)) {
                ...
                if (c == '=') {
                  ...
                  #if DUMMY_SEC_FROM_PATTERN
                  if (!dummy_sec)
                    dummy_sec = make_dummy_sec_from_pattern(gv);
                  #else
                  if (!dummy_sec) {
